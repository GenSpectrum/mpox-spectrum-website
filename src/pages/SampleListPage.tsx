...
export const SampleListPage = () => {
    ...
    function handleDownloads(type: string, infoText: string, dataFormat: string = '') {
        for (let i = 0; i < accessions.length; i++) {
            let fileURL = dataFormat === 'csv' ? getLinkTo(type, accessions[i], undefined, true, 'csv') : getLinkTo(type, accessions[i], undefined, true);
            let tempLink = document.createElement('a');
            tempLink.href = fileURL;
            alert(` Downloading ${infoText} for ${accessions[i].identifier.accession}`)
            tempLink.click()
        }
    }
    ...
    useEffect(() => {
        if (selectionModel.length > 0 && rows) {
            let selected = selectionModel.map((i: number) => {
                return {"identifier": {"accession": rows[i].accession}}
            })
            setAccessions(selected)
        }
    }, [selectionModel, rows])
    ...
    return (
        <>
            {selectionModel.length > 0 &&  <div className='m-8'>
